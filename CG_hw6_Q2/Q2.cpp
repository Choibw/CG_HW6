...
// Gouraud용: 각 정점마다 미리 계산한 색을 저장
static std::vector<glm::vec3> gColorBuffer;
// -------------------------------------------------
...
void rasterizeTriangle(const glm::vec4& a, const glm::vec4& b, const glm::vec4& c, const glm::uvec3& tri) // 증분식 구현
{
	...
	// tri에 대응하는 정점 색 가져오기
	glm::vec3 c0 = gColorBuffer[tri.x];
	glm::vec3 c1 = gColorBuffer[tri.y];
	glm::vec3 c2 = gColorBuffer[tri.z];
	...
	for (int y = minY; y <= maxY; ++y)
	{
		for (int x = minX; x <= maxX; ++x)
		{
			...
			if (w0 >= 0 && w1 >= 0 && w2 >= 0) // 내부
			{
				...
				glm::vec3 color = w0 * c0 + w1 * c1 + w2 * c2; // Gouraud shading
...
void render()
{
	...
	gColorBuffer.clear();
	gColorBuffer.reserve(gVertexBuffer.size());
	for (size_t i = 0; i < gVertexBuffer.size(); ++i) {
		// 1) 객체 공간 법선 (unit sphere)
		glm::vec3 n_obj = glm::normalize(gVertexBuffer[i]);
		// 2) 뷰 공간 노멀
		glm::vec3 normal_vs = glm::normalize(glm::vec3(V * M * glm::vec4(n_obj, 0.0f)));
		// 3) 뷰 공간 위치
		glm::vec3 pos_vs = glm::vec3(V * M * glm::vec4(gVertexBuffer[i], 1.0f));
		// 4) 뷰 방향
		glm::vec3 viewDir = glm::normalize(glm::vec3(0.0f) - pos_vs);
		// 5) 조명 계산
		glm::vec3 c = computeLighting(
